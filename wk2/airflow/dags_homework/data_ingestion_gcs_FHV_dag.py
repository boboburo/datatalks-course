...
OUTPUT_CSV_FILE_TEMPLATE = AIRFLOW_HOME + '/raw_csv/fhv_{{ execution_date.strftime(\'%Y-%m\') }}.csv'
OUTPUT_PARQUET_FILE_TEMPLATE = AIRFLOW_HOME + '/parquet/fhv_{{ execution_date.strftime(\'%Y-%m\')}}.parquet'
...
gcs_workflow = DAG(
    dag_id = "FHV_GCSIngestionDag",
    schedule_interval = "0 6 1 * *", 
    start_date=datetime.datetime(2019, 1, 1),
    end_date=datetime.datetime(2019,12,2),
    )
...
with gcs_workflow:
    ...
    local_to_gcs_task = PythonOperator(
        ...
        op_kwargs={
            "bucket": BUCKET,
            "object_name": "fhv/" + OUTPUT_PARQUET_FILE_TEMPLATE.split("/")[-1],
