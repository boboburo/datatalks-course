...
OUTPUT_CSV_FILE_TEMPLATE = AIRFLOW_HOME + '/raw_csv/output_{{ execution_date.strftime(\'%Y-%m\') }}.csv'
OUTPUT_PARQUET_FILE_TEMPLATE = AIRFLOW_HOME + '/parquet/output_{{ execution_date.strftime(\'%Y-%m\')}}.parquet'
...
# create the dag could add in retries here ? 

gcs_workflow = DAG(
    dag_id = "YT_GCSIngestionDag",
...
with gcs_workflow:
    ...
    local_to_gcs_task = PythonOperator(
        ...
        op_kwargs={
            "bucket": BUCKET,
            "object_name": "yellow_trip/" + OUTPUT_PARQUET_FILE_TEMPLATE.split("/")[-1],
